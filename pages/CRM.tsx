...
const CRM: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        const data = await mockDb.getLeads();
        setLeads(data);
        setLoading(false);
    };
    fetchData();
  ...
  const handleAddLead = async (e: React.FormEvent) => {
    e.preventDefault();
    const lead = await mockDb.addLead({ ...newLead, status: 'New Lead' });
    setLeads([...leads, lead]);
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    if (confirm('Are you sure?')) {
        await mockDb.deleteLead(id);
        setLeads(leads.filter(l => l.id !== id));
        if (selectedLead?.id === id) setSelectedLead(null);
    }
  };

  const moveLead = async (id: string, currentStatus: string, direction: 'next' | 'prev', e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    if (newIndex >= 0 && newIndex < COLUMNS.length) {
        ...
        await mockDb.updateLead(id, { status: newStatus });
